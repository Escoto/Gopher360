...
void Gopher::loadConfigFile()
{
	...
	if (speeds.size() == 0)
	{
		speeds.push_back(0.01f);
		speeds.push_back(0.07f);
...
void Gopher::loop()
{
	...
	{
		const int CHANGE_SPEED_VIBRATION_INTENSITY = 40000;   // Speed of the vibration motors when changing cursor speed.
		const int CHANGE_SPEED_VIBRATION_DURATION = 400;      // Duration of the cursor speed change vibration in milliseconds.
...
void Gopher::setXboxClickState(DWORD STATE)
{
	...
	if (_xboxClickIsDownLong[STATE])
	{
		...
		if (*it == GAMEPAD_DPAD_UP)
		{
			Sleep(15);
			inputKeyboardDown(GAMEPAD_DPAD_UP);
		}
		else if (*it == GAMEPAD_DPAD_DOWN)
		{
			Sleep(15);
			inputKeyboardDown(GAMEPAD_DPAD_DOWN);
		}
	}
}
